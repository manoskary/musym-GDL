...
class MyGraph(object):
    ...
    def adj(self):
        ...
        coo = torch.sparse_coo_tensor(self.edge_index, ones, (self.x.shape[0], self.x.shape[0]))
        return coo.to_dense()
...
def main():
    ...
    model = GraphNvpModel(in_feats, 16, in_feats, 3, 0.5)
    criterion = FlowLikelihood()
...
def flowtrain():
    graphs = MyGraphDataset(
        root_dir='/home/manos/Desktop/JKU/codes/tonnetzcad/node_classification/cad-feature-wtc').graphs
    in_feats = graphs[0].x.shape[1]
    gdata = [(g.x, g.adj()) for g in graphs]
    train_flow(GraphFlowLight(GraphFlow(in_feats, 3)), gdata)
