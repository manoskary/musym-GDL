...
def dgl2pyg(dgl_graph):
    ...
    labels = dgl_graph.ndata.pop("label")
    dgl_graph = to_bidirected(dgl_graph)
    adj = dgl_graph.adj()
    pyg_graph = Data(x=features, edge_index=adj, y=labels)
    return pyg_graph
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default="cad_basis_homo")
    parser.add_argument('--max-deg', type=int, default=100)
    ...
    g, n_classes = load_and_save("cad_basis_homo", os.path.abspath("/share/home/manos/codes/musym-GDL/musym/models/rgcn_homo/data/"))
    data = dgl2pyg(g)
